...
class Payment:
    ...
    def get_fee(self):
        ...
        def card_service_type(possible_fees):
            ...
            result = []
            for f in possible_fees:
                if self.parsed['card_service_type'] is not None and f['card_service_type'] == self.parsed[
                        'card_service_type']:
                    result.append(f)
        ...
        def return_final_fee(result_fees):
            ...
            # more than 1 fee applicable
                ...
                raise Exception(
                    "Error: there are " + str(len(result_fees)) + " fees applicable for the payment: \n\n" + str(
                        self.parsed) + "\n\nDostupné fees:\n" + str(result_fees))
    ...
    def get_exception_multiplier(self):
        ...
        if self.parsed['partnership_id'] not in self.exceptions:
            ...
            for i in self.exceptions[self.parsed['partnership_id']]:
                ...
                if i['date_from'] <= date_tested <= i['date_to']:
    ...
    def process_payment(self):
        ...
        # schema cost multiplier - i.e. scheme costs of skylink payments are being divided between TP and GP
        schema_cost_multiplier = self.get_exception_multiplier()
        ...
        fee_cur = self.parsed['currency'] if self.fee['transaction_fee_currency'] is None else self.fee[
        ...
        # amounts
        self.parsed['amount'] = D(0).quantize(DECIMAL_PLACES) if self.parsed['amount'] is None else self.parsed[
            'amount'].quantize(DECIMAL_PLACES)
        self.parsed['amount_czk'] = D(0).quantize(DECIMAL_PLACES) if self.parsed['amount'] is None else (
                self.parsed['amount'] * czk_rate).quantize(DECIMAL_PLACES)
        self.parsed['amount_refunded'] = D(0).quantize(DECIMAL_PLACES) if self.parsed['amount_refunded'] is None else \
            self.parsed['amount_refunded'].quantize(DECIMAL_PLACES)
        self.parsed['amount_refunded_czk'] = D(0).quantize(DECIMAL_PLACES) if self.parsed[
                                                                                  'amount_refunded'] is None else (
                self.parsed['amount_refunded'] * czk_rate).quantize(DECIMAL_PLACES)

        # costs
        if self.fee['cost_algorithm'] == 'STD':
            self.parsed['cost_algorithm'] = 'STD'
            self.parsed['provider_transaction_fee'] = D(0).quantize(DECIMAL_PLACES) if self.fee[
                                                                            'transaction_fee'] is None else (
                    self.fee['transaction_fee'] * rate_fee).quantize(DECIMAL_PLACES)
            self.parsed['provider_transaction_fee_czk'] = (
                    self.parsed['provider_transaction_fee'] * czk_rate_fee).quantize(DECIMAL_PLACES)
            self.parsed['provider_percent_fee'] = (self.parsed['amount'] * (self.fee['fee'] / 100)).quantize(
                DECIMAL_PLACES)
            self.parsed['provider_percent_fee_czk'] = (self.parsed['amount_czk'] * (self.fee['fee'] / 100)).quantize(
                DECIMAL_PLACES)
            ...
            self.parsed['total_fee'] = (
                    self.parsed['provider_transaction_fee'] + self.parsed['provider_percent_fee']).quantize(
                DECIMAL_PLACES)
            self.parsed['total_fee_czk'] = (
                    self.parsed['provider_transaction_fee_czk'] + self.parsed['provider_percent_fee_czk']).quantize(
                DECIMAL_PLACES)
            ...
            transaction_fee = D(0).quantize(DECIMAL_PLACES) if self.fee['transaction_fee'] is None else (
                    self.fee['transaction_fee'] * rate_fee).quantize(DECIMAL_PLACES)
            ...
            self.parsed['provider_transaction_fee'] = D(0).quantize(DECIMAL_PLACES) if self.fee[
                                                                                   'transaction_fee'] is None else (
                    self.fee['transaction_fee'] * rate_fee).quantize(DECIMAL_PLACES)
            self.parsed['provider_transaction_fee_czk'] = D(0).quantize(DECIMAL_PLACES) if self.fee[
                                                                                       'transaction_fee'] is None else (
                    self.fee['transaction_fee'] * czk_rate_fee).quantize(DECIMAL_PLACES)
            self.parsed['provider_percent_fee'] = (self.parsed['amount'] * (self.fee['fee'] / 100)).quantize(
                DECIMAL_PLACES)
            self.parsed['provider_percent_fee_czk'] = (self.parsed['provider_percent_fee'] * czk_rate).quantize(
                DECIMAL_PLACES)
            self.parsed['interchange_fee'] = self.parsed['interchange_fee'].quantize(
                DECIMAL_PLACES) * schema_cost_multiplier if self.parsed['interchange_fee'] is not None else D(
                0).quantize(DECIMAL_PLACES)
            self.parsed['association_fee'] = self.parsed['association_fee'].quantize(
                DECIMAL_PLACES) * schema_cost_multiplier if self.parsed['association_fee'] is not None else D(
                0).quantize(DECIMAL_PLACES)
            ...
            self.parsed['total_fee'] = (
                    self.parsed['provider_transaction_fee'] + self.parsed['provider_percent_fee'] + self.parsed[
                        'interchange_fee'] + self.parsed['association_fee']).quantize(DECIMAL_PLACES)
            self.parsed['total_fee_czk'] = (
                    self.parsed['provider_transaction_fee_czk'] + self.parsed['provider_percent_fee_czk'] +
                    self.parsed['interchange_fee_czk'] + self.parsed['association_fee_czk']).quantize(
                DECIMAL_PLACES)
            ...
            self.parsed['provider_transaction_fee_czk'] = D(0).quantize(DECIMAL_PLACES) if self.fee[
                                                                                       'transaction_fee'] is None else (
                    self.fee['transaction_fee'] * czk_rate_fee).quantize(DECIMAL_PLACES)
            self.parsed['provider_percent_fee'] = ((self.parsed['amount'] * (self.fee['fee'] / 100)) - (
                    self.parsed['interchange_fee'] + self.parsed['association_fee'])).quantize(DECIMAL_PLACES)
            self.parsed['provider_percent_fee_czk'] = ((self.parsed['amount_czk'] * (self.fee['fee'] / 100)) - (
                    self.parsed['interchange_fee_czk'] + self.parsed['association_fee_czk'])).quantize(
                DECIMAL_PLACES)
            self.parsed['total_fee'] = ((self.parsed['amount'] * (self.fee['fee'] / 100)) + self.parsed[
                'provider_transaction_fee']).quantize(DECIMAL_PLACES)
            self.parsed['total_fee_czk'] = ((self.parsed['amount_czk'] * (self.fee['fee'] / 100)) + self.parsed[
                'provider_transaction_fee_czk']).quantize(DECIMAL_PLACES)
